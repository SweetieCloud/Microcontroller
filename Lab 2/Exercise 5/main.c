...
int main(void)
{
  ...
  // Vòng lặp chính thực hiện logic đồng hồ số theo Program 1.5 (ĐÃ SỬA)
  while (1)
  {
    // Tăng giây, phút, giờ
    second ++;
    ...
    // Cập nhật giá trị giờ và phút vào led_buffer
    updateClockBuffer ();

    // Dừng 1 giây (HAL_Delay() là hàm bận, nhưng đáp ứng yêu cầu đề bài)
    HAL_Delay (1000) ;
  /* USER CODE END WHILE */
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {
    ...
    if (blink_counter <= 0) {
        ...
        // Lưu ý: DOT_Pin (giữa giờ và phút) cần được BẬT ở đây.
        // Tuy nhiên, logic này sẽ nhấp nháy DOT mỗi giây.
