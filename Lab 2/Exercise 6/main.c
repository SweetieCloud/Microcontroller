...
int blink_counter = 100; // T_blink = 1s
int index_led = 0;
/* USER CODE END PV */
...
// BỘ ĐẾM VÀ HÀM CỦA SOFTWARE TIMER (PROGRAM 1.6)
int timer0_counter = 0;
int timer0_flag = 0;
int TIMER_CYCLE = 10; // Chu kỳ ngắt Timer 2 là 10ms

/**
 * @brief Đặt thời gian (duration) cho Software Timer
...
/**
 * @brief Giảm bộ đếm và đặt flag khi đếm hết
 * @retval None
 */
void timer_run (){
    if( timer0_counter > 0){
 ...
 */
void updateClockBuffer(void) {
    // 1. Xử lý Giờ (HH) -> led_buffer[0] (Hàng chục) và led_buffer[1] (Hàng đơn vị)
    led_buffer[0] = hour / 10;
    led_buffer[1] = hour % 10;

    // 2. Xử lý Phút (MM) -> led_buffer[2] (Hàng chục) và led_buffer[3] (Hàng đơn vị)
...
void display7SEG(int num) {
    ...
    switch (num) {
        ...
        case 8:
            ...
            break;
        case 9:
            // LỖI Ở ĐÂY: Thiếu d_Pin (Đoạn dưới cùng)
            HAL_GPIO_WritePin(GPIOB, a_Pin|b_Pin|c_Pin|d_Pin|f_Pin|g_Pin, GPIO_PIN_RESET); // ĐÃ SỬA: Thêm d_Pin
            break;
        default:
...
int main(void)
{
  ...
  // KHỞI TẠO SOFTWARE TIMER: Đặt chu kỳ 1 giây (1000ms) cho nhịp đồng hồ
  setTimer0 (1000);
  /* USER CODE END 2 */
...
void SystemClock_Config(void)
{
  ...
  */
  ...
  // Dòng 236 ĐÃ SỬA: Thêm tiền tố 'RCC_'
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {
    ...
    timer_run(); // GỌI THÊM: Cập nhật Software Timer mỗi 10ms (Program 1.7)
    ...
    // Giữ lại logic này để DOT nhấp nháy đồng bộ với nhịp đồng hồ 1 giây
    blink_counter--;
    if (blink_counter <= 0) {
