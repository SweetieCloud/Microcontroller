...
/* USER CODE BEGIN PV */
// Giá trị đã điều chỉnh cho Bài tập 4:
// scan_counter = 25: T_S = 25 * 10ms = 250ms. T_total = 4 * 250ms = 1000ms (1Hz)
// blink_counter = 100: Chu kỳ nháy 100 * 10ms = 1000ms (1 giây)
int led_buffer[MAX_LED] = {1, 2, 3, 4}; // Dữ liệu hiển thị trên 4 LED
int scan_counter = 25;
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {
    ...
    scan_counter--;
    if (scan_counter <= 0) {
        // ĐÃ SỬA: Đặt lại bộ đếm cho 250ms (T_S = 25 * 10ms)
        scan_counter = 25;
    ...
    // Không cần sửa, vì blink_counter = 100 vẫn giữ nguyên chu kỳ 1s
    blink_counter--;
    if (blink_counter <= 0) {
